...
begin
  ...
  if IsNaND(x) then sfd_e1 := x
    ...
    y := PolEval(x,P01,6) / PolEval(x,Q01,7);
    sfd_e1 := y + (x - ln(x) - y01);
  end
  else if x>=ln_MaxDbl then sfd_e1 := 0.0
  else begin
    y := 1.0/x;
    z := 1.0 + PolEval(y,Pix,14) / PolEval(y,Qix,14);
    sfd_e1 := z*(exp(-x)*y);
  end;
end;
...
function sfd_en(n: longint; x: double): double;
  {-Return the exponential integral E_n(x) = integral(exp(-x*t)/t^n, t=1..Inf), x > 0}
begin
  if n=0           then sfd_en := exp(-x)/x {HMF [1], 5.1.24}
