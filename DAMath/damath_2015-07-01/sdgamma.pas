...
(*************************************************************************
 ...
 1.16.00  15.12.14  we          small changes in sfc_batemang
...
const
  ...
  NXPGMAX = 150;                     {max. n for polygamma(n,x) with x < 0}
...
function sfd_polygamma(n: integer; x: double): double;
  {-Return the polygamma function: n'th derivative of psi; n>=0, x>0 for n>NXPGMAX.}
...
var
  BNN: array[0..7] of double absolute BNNHex;
...
function polygam_negx(n: integer; x: double): double;
  {-Polygamma for negative x and 1 <= n <= NXPGMAX}
type
  TCArray = array[-1..NXPGMAX+2] of double;
  PCArray = ^TCArray;
...
begin
  sincosPi(x,s,c);
  if (n<1) or (n>NXPGMAX) or (x>=0.0) or (s=0.0) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    polygam_negx := NaN_d;
  ...
  if t > ln_MaxDbl then z := PosInf_d
    ...
    if (abs(z)<709.0) and (n<MAXGAMD-1) then begin
      ...
      if n <= NPMAX then begin
        {avoid exp/ln inaccuracies, example: sfd_polygamma(1800,980)}
        f := 1.0;
...
begin
  ...
  else if (x<0.0) and (n>NXPGMAX) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfd_polygamma := NaN_d;
