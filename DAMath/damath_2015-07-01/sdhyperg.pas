...
(*************************************************************************
 ...
 REFERENCES    :  References used in this unit, main index in damath_info.txt/references
                 ...
                 [55] N.M. Temme, The Numerical Computation of the Confluent Hypergeometric
                      ...
                      or http://oai.cwi.nl/oai/asset/10717/10717A.pdf
...
begin
  ...
  if x<0 then begin
    ...
    {test cases, so use the safe path and get a little increased inaccuracy.}
    if x>1.0 then begin
      a0 := sfd_lngammas(b,j);
      a1 := sfd_lngammas(a,k);
      a2 := (a-b)*ln(x);
...
var
  p,q,r,s,t,y,d,ax,d1,d2,e,y1,l2: double;
  i,id,aid: longint;
  s1,s2,s3,s4: integer;
const
...
begin
  ...
  if x <= 0.75 then begin
    ...
    if abs(d-rint(d)) > IEPS then begin
      ...
      h2f1_trans := h2f1_sum(a, b, c, x, loss);
      if abs(loss) < 0.25*ETHRESH then exit;
      {If power series fails, then apply reflection formula HMF[1] 15.3.6}
...
begin
  ...
  if x<0.0 then begin
    ...
    hyp2f1e := r*t;
    {$ifndef VER50}
      {Current Luke too complicated for brain-damaged TP5}
      if err>ETHRESH then begin
...
begin
  if IsNanOrInfD(a) or IsNanOrInfD(x) or (a<0.0) or (frac(2.0*a)<>0) or (a>amax) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfd_pcfv := NaN_d;
