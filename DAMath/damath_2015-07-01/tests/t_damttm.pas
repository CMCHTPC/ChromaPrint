{Main test unit DAMTools  (c) W.Ehrhardt 2010-2013}
...
begin
  ...
  writeln('Test program for DAMTools unit    (c) W.Ehrhardt 2010-2013');
  writeln('Testing:  zbrent/zeroin and localmin/mbrent/fmin');
  ...
  for i:=2 to 50 do begin
    ...
    if abs(x-y)>5*10*eps_d*y then begin
      inc(totalfail);
      writeln('zeroin fz3: ',n:3,1-x/y:30);
    end;
  end;
  ...
  for i:=1 to 20 do begin
    ...
    if abs(x-y)>12*eps_d*y then begin
      inc(totalfail);
      writeln('zeroin fz2: ',n:3,1-x/y:30);
    end;
  end;
  ...
  for i:=1 to 50 do begin
    ...
    if abs(x-y)>6*eps_d*y then begin
      inc(totalfail);
      writeln('zeroin fz1/a=0.25: ',n:3,1-x/y:30);
    end;
    a := 1;
    ...
    if abs(x-y)>6*eps_d*y then begin
      inc(totalfail);
      writeln('zeroin fz1/a=1: ',n:3,1-x/y:30);
    end;
  end;
...
(*
  ...
  localmin({$ifdef FPC_ProcVar}@{$endif}gammax,1,2,sqrt(eps_d), eps_d, x,y,i);
  inc(totalcnt);
...
begin
  ...
  for i:=0 to 16 do begin
    alpha := int(i)/8.0;        {FPC64 nonsense 'feature'}
  ...
  for i:=-10 to 10 do begin
    alpha := int(i)/5.0;       {FPC64 nonsense 'feature'}
...
begin
  ...
  for i:=0 to 10 do begin
    {jump at alpha}
    alpha := int(i)/10.0;  {FPC64 nonsense 'feature'}
    ans := 2.0-alpha;
...
begin
  ...
  for i:=-15 to 32 do begin
    c := int(i)/16;    {FPC64 nonsense 'feature'}
  ...
  for i:=-4 to 17 do begin
    if i<>0 then begin
      b := int(i)/16;    {FPC64 nonsense 'feature'}
...
begin
  ...
  for i:=1 to 15 do begin
    alpha := int(i)/16.0;     {FPC64 nonsense 'feature'}
    ans := Pi/sinPi(alpha);
  ...
  for i:=1 to 16 do begin
    alpha := int(i)/2.0;      {FPC64 nonsense 'feature'}
    ans := sqrt(Pi_2/alpha);
  ...
  for i:=1 to 16 do begin
    alpha := int(i)/4.0;      {FPC64 nonsense 'feature'}
    ans := Pi_2*exp(-alpha);
  ...
  for i:=1 to 16 do begin
    alpha := int(4*i)/7.0;   {FPC64 nonsense 'feature'}
...
procedure test_amint;
  {-Test numerical integration}
begin
  writeln('Test DAMTools/numerical integration    (c) W.Ehrhardt 2011-2013');
  totalfail := 0;
  totalcnt  := 0;
  test_quanc8;
  test_quagk;
