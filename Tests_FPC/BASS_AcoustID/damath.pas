...
function langevin(x: double): double;
...
{#Z+}
{------------------------------------------------------------------}
{-------------  Basic dbl2 (double-double) functions  -------------}
{------------------------------------------------------------------}
{#Z-}
...
function ln(x: double): double;
  {-Return natural logarithm of x, x may be denormal}
begin
  {It seems that all supported compilers correctly handle +Inf, Nan, Denormal}
  ln := system.ln(x);
end;
...
begin
  if THexDblW(x)[3] and $7FF0=$7FF0 then begin
    {Inf or NaN}
    if IsNaND(x) or (x>0.0) then log10 := x
    else log10 := Nan_d;
    exit;
  end;
  log10 := ln(x)*log10e;
...
begin
  if THexDblW(x)[3] and $7FF0=$7FF0 then begin
    {Inf or NaN}
    if IsNaND(x) or (x>0.0) then log2 := x
    else log2 := Nan_d;
    exit;
  end;
  log2 := ln(x)*log2e;
...
function logbase(b, x: double): double;
  {-Return base b logarithm of x}
begin
  if (THexDblW(x)[3] and $7FF0=$7FF0) or (THexDblW(b)[3] and $7FF0=$7FF0) then begin
    logbase := Nan_d;
  end
  else logbase := ln(x)/ln(b);
end;
...
begin
  ...
  if (i=$7FF) or (e=0) or (x=0.0) then ldexpd := x
    ...
    if e>$7FE then begin
      {overflow}
      if x>0.0 then ldexpd := {PosInf_d} 2.0*MaxDouble
      else ldexpd := {NegInf_d}(-2.0)*MaxDouble;
    end
...
begin
  ...
  if z < 38.0 then begin
    if z < 1.25e-2 then begin
      ...
      if z < 1e-4 then gd := x*(1.0-sqr(x)/6.0)
      else begin
        z  := x*x;
        gd := x*(1.0 - z/6.0*(1.0 - 0.25*z*(1.0 - 61.0*z/210.0)));
      end;
    end
...
begin
  if THexDblW(x)[3] and $7FF0=$7FF0 then begin
    {Inf or NaN}
    if x=PosInf_d then ln1pmx := -x else ln1pmx := x;
    exit;
  end;
...
begin
  ...
  if x < 6.15e-4 then begin
    if x < 2.3e-10 then sinc := 1.0
      ...
      x := sqr(x);
      if x < 2.3e-10 then sinc := 1.0 - x/6.0
      else sinc := 1.0 - (x/6.0 - sqr(x)/120.0)
    end;
  end
  else sinc := sin(x)/x;
end;
...
begin
  ...
  if w=y then begin
    ...
    if (z<512) and (z*(abs(u) + 1.0) < 1022.0) then begin
...
begin
  if THexDblW(x)[3] and $7FF0=$7FF0 then begin
    {Inf or NaN}
    if x=PosInf_d then arccosh := x else arccosh := Nan_D;
    exit;
  end;
...
begin
  if THexDblW(x)[3] and $7FF0=$7FF0 then begin
    {Inf or NaN}
    if x=PosInf_d then arccosh1p := x else arccosh1p := Nan_D;
    exit;
  end;
...
begin
  ...
  t := abs(x);
  if t <= 1.0 then langevin := x*CSEvalD(2.0*sqr(x)-1.0, lvcs, nlv)
  else begin
